import { Component } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { CommonModule } from '@angular/common';
import { EditProfileComponent } from "./edit-profile/edit-profile.component";

@Component({
  selector: 'app-profile',
  standalone: true,
  imports: [FormsModule, CommonModule, EditProfileComponent],
  templateUrl: './profile.component.html',
  styleUrl: './profile.component.css'
})
export class ProfileComponent {
  isSelect=false
  isEditMode = false;
  isDropdownOpen = false;
  selectedCorporates: { id: string; name: string }[] = [];
  childData:any={}
  companyDetails:any={}
  availableCorporates = [
    { id: 'corp1', name: 'Veyronix Technologies' },
    { id: 'corp2', name: 'Truvara Solutions' },
    { id: 'corp3', name: 'TechCorp Inc' },
    { id: 'corp4', name: 'Digital Solutions Ltd' },
    { id: 'corp5', name: 'Innovation Hub' },
    { id: 'corp6', name: 'Future Systems' }
  ];

  ngOnInit(){
    this.companyDetails ={
      corporateName: 'Alma Travel',
      buildingNumber: '2768',
      streetName: 'Prince Saud Al Faisal',
      district: 'Al Khalidiyyah',
      city: 'Jeddah',
      postalCode: '23422',
      unitNumber: '',
      country: 'Saudi Arabia',
      currency: 'SAR',
      timezone: '(UTC+03:00) Kuwait, Riyadh',
      logoFile: [null],
      logoUrl: "data:image/jpeg;base64,"
    };
  }

  toggleEdit() {
    this.isSelect = !this.isSelect;
  }

  toggleDropdown() {
    this.isDropdownOpen = !this.isDropdownOpen;
    this.isSelect=true
  }

  addCorporate(corporateId: string) {
    if (!this.isCorporateSelected(corporateId)) {
      const corporate = this.availableCorporates.find(corp => corp.id === corporateId);
      if (corporate) {
        this.selectedCorporates.push(corporate);
      }
    }
    this.isDropdownOpen = false;
  }

  removeCorporate(corporateId: string, event: Event) {
    event.stopPropagation();
    this.selectedCorporates = this.selectedCorporates.filter(corp => corp.id !== corporateId);
  }

  isCorporateSelected(corporateId: string): boolean {
    return this.selectedCorporates.some(corp => corp.id === corporateId);
  }

  saveExclusions() {
    if (this.selectedCorporates.length > 0) {
      console.log('Saving exclusions for corporates:', this.selectedCorporates);
      // Add your save logic here
    } else {
    }
  }
  receiveData(data: string) {
    if(data!=null){
      this.companyDetails = data;
    }

    this.isEditMode=false
    console.log('استلمت الداتا:', data);
  }
}
